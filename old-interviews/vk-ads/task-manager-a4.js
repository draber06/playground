...
class TaskManager {
    constructor(N) {
        ...
        this.reports = Array.from({ length: N }, () => ({
            successCount: 0,
            failedCount: 0,
            tasks: [],
            timeSpent: 0,
    ...
    run = async () => {
        // Sort once before running (descending priority)
        this.queue.sort((a, b) => a.priority - b.priority);

        const workers = [];
        for (let i = 0; i < this.robotCount; i++) {
            workers.push(this.assignTasksToRobot(i));
        }
    ...
    async assignTasksToRobot(robotIndex) {
        const report = this.reports[robotIndex];
        ...
        while (this.queue.length > 0) {
            ...
            try {
                ...
                report.timeSpent += Date.now() - startTime;
            }
        }
    }
}
